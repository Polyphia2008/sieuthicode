...
--
-- Quy tắc:
--  - Chỉ promote khi hệ thống KHÔNG còn superadmin ĐANG HOẠT ĐỘNG (banned = 0).
--    Một superadmin đã bị ban KHÔNG được làm migration bỏ qua mãi mãi — nếu
--    không còn superadmin active nào, phải promote admin active id nhỏ nhất.
...
--  - Idempotent: chạy hai lần liên tiếp — lần thứ hai không promote thêm vì
--    đã có superadmin active (điều kiện NOT EXISTS không còn khớp).
...
AND NOT EXISTS (
    SELECT 1 FROM (
        SELECT `id` FROM `users` WHERE `level` = 'superadmin' AND `banned` = 0 LIMIT 1
    ) AS `existing`
);
