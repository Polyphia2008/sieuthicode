<?php
// statically decompiled from viewminigame.php  [structured; all 1 record(s) structured]

require_once realpath($_SERVER['DOCUMENT_ROOT']) . '/libs/init.php';
if (!$user) {
    new Redirect('/');
    exit();
} else {
    if (isset($_GET['transid'])) {
        $code = Anti_xss($_GET['transid']);
        $info = $db->get_row(' SELECT * FROM `spin_quest_logs` WHERE `trans_id` = \'' . $code . '\' AND `user_id` = \'' . $data_user['id'] . '\' ');
        if (!$info) {
            new Redirect('/customer/history/minigame');
        }
    } else {
        new Redirect('/customer/history/minigame');
    }
    $title = 'Thông tin đơn hàng - ' . $db->site('title');
    require_once realpath($_SERVER['DOCUMENT_ROOT'] . '/views/header.php');
    echo '<div class="breadCrumbs">
    <div class="screen">
        <div class="center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a class="text-decoration-none" href="/"><span>Trang chủ</span></a></li>
                <li class="breadcrumb-item active"><a class="text-decoration-none" href=""><span>Chi tiết lịch sử chơi minigame</span></a></li>
            </ol>
        </div>
    </div>
</div>
<div class="screen">
    <div class="center">
        <div class="page-account">
            <div class="menu-account">
                ';
    require_once realpath($_SERVER['DOCUMENT_ROOT'] . '/views/profile/menu.php');
    echo '            </div>
            <div class="container-account" id="container-account">
                <div class="content-account header-desktop-account">
                    <h2 class="title-account">Lịch sử chơi minigame #(';
    echo $info['trans_id'];
    echo ')</h2>
                    <hr>
                </div>
                <div class="content-account header-mobile-account">
                    <div class="div-header">
                        <a href="account/lich-su-nap-the"><img src="assets/images/images/sidebar_arrow_right.svg" alt=""></a>
                        <h2 class="title-account">Chi tiết lịch sử chơi minigame</h2>
                    </div>
                </div>
                <div class="content-account">
                    <h2 class="title-sub-account">Minigame ';
    echo $info['name_spin'];
    echo '</h2>
                    <hr>
                    <h4 class="mt-3">Thông tin giao dịch</h4>
                    <div class="container-content-account mt-3">
                        <div class="items-content-account">
                            <div>
                                <p>Mã GD</p>
                                <P>#';
    echo $info['trans_id'];
    echo '</P>
                            </div>
                            <div>
                                <p>Danh mục</p>
                                <P>';
    echo $info['name_spin'];
    echo '</P>
                            </div>
                        </div>
                        <div class="items-content-account">
                            <div>
                                <p>Thanh toán</p>
                                <P>';
    echo format_cash($info['price']);
    echo 'đ</P>
                            </div>
                            <div>
                                <p>Ngày giao dịch</p>
                                <P>';
    echo $info['created_at'];
    echo '</P>
                            </div>
                            <div>
                                <p>Trạng thái</p>
                                <P><span class=\'text-success\'>Thành công</span></P>
                            </div>
                        </div>
                        <div class="items-content-account">
                            <div>
                                <p>Phần thưởng quay trúng</p>
                                <P>';
    echo $info['content'];
    echo '</P>
                            </div>
                          
                        </div>

                        <a href="/customer/history/minigame" class="button-trove-content">Trở về</a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
';
    require_once realpath($_SERVER['DOCUMENT_ROOT'] . '/views/footer.php');
}
