...
$manualTasks = $db->get_list("SELECT m.*,c.status AS claim_status FROM traffic_manual_tasks m LEFT JOIN traffic_manual_claims c ON c.task_id=m.id AND c.user_id='".$userId."' WHERE m.status=1 ORDER BY m.id DESC");
$statusLabels = [
...
<?php endif; ?>
  <header class="traffic-banner">
    <img class="traffic-banner-gif" src="https://tienthanhsub.me/assets1/img/45f6fd99f154c61836a77f5824cb5684.gif" alt="" aria-hidden="true" referrerpolicy="no-referrer">
  ...
  <?php if($manualTasks):?><section class="premium-task-section"><div class="traffic-section-title"><div><span>NHIỆM VỤ ĐẶC BIỆT</span><h2>Nhiệm vụ thưởng cao</h2></div><i class="fas fa-crown"></i></div><p class="premium-intro">Nhận nhiệm vụ và tham gia nhóm Zalo để được hướng dẫn. Admin sẽ kiểm tra và duyệt thưởng thủ công.</p><div class="premium-task-grid"><?php foreach($manualTasks as $manual):?><article class="premium-task-card"><div class="premium-card-glow"></div><div class="premium-card-top"><span><i class="fas fa-gem"></i> THƯỞNG CAO</span><strong>+<?=format_cash($manual['reward'])?>đ</strong></div><h3><?=htmlspecialchars($manual['title'],ENT_QUOTES)?></h3><p><?=nl2br(htmlspecialchars($manual['description'],ENT_QUOTES))?></p><div class="premium-review-time"><i class="far fa-clock"></i> Duyệt: <?=htmlspecialchars($manual['review_wait'],ENT_QUOTES)?></div><button type="button" class="premium-claim-btn" data-task-id="<?=$manual['id']?>" data-join-url="<?=htmlspecialchars($manual['join_url'],ENT_QUOTES)?>"><i class="fas fa-hand-pointer"></i> Nhận nhiệm vụ</button></article><?php endforeach;?></div></section>
<div class="manual-task-modal" id="manual-task-modal" aria-hidden="true"><div class="manual-modal-card" role="dialog" aria-modal="true" aria-labelledby="manual-modal-title"><button class="manual-modal-close" type="button" aria-label="Đóng">&times;</button><span class="manual-modal-icon"><i class="fas fa-users"></i></span><h3 id="manual-modal-title">Tham gia nhóm Zalo để làm nhiệm vụ</h3><p>Hãy tham gia nhóm Zalo của chúng tôi để được admin hướng dẫn chi tiết cách hoàn thành nhiệm vụ thưởng cao.</p><a href="https://zalo.me/g/ebhzvqycdb5wgsftdeai" target="_blank" rel="noopener" class="manual-zalo-link">https://zalo.me/g/ebhzvqycdb5wgsftdeai</a><button type="button" class="manual-modal-continue">Đã hiểu — Tham gia nhóm</button></div></div><?php endif;?>

  <section class="traffic-wallet-section">
    <div class="traffic-section-title"><div><span>VÍ NHIỆM VỤ</span><h2>Quản lý tiền thưởng</h2></div><i class="fas fa-wallet"></i></div>
    <div class="traffic-wallet-grid">
      <article class="wallet-balance-card"><small>Số dư có thể sử dụng</small><strong><?=format_cash((int)$trafficWallet['balance'])?>đ</strong><p>Bạn có thể chuyển sang số dư chính hoặc gửi yêu cầu rút ngân hàng.</p></article>
      <article class="wallet-action-card"><h3>Chuyển vào số dư chính</h3><form class="traffic-wallet-form"><input type="hidden" name="csrf_token" value="<?=htmlspecialchars(generate_csrf_token(),ENT_QUOTES)?>"><input type="hidden" name="action" value="transfer"><input type="number" min="1" <?php if((int)$trafficWallet['balance']>0):?>max="<?=(int)$trafficWallet['balance']?>"<?php endif;?> name="amount" placeholder="Số tiền muốn chuyển" required <?=((int)$trafficWallet['balance']<=0?'disabled':'')?>><button <?=((int)$trafficWallet['balance']<=0?'disabled':'')?>><?=((int)$trafficWallet['balance']<=0?'Chưa có số dư':'Chuyển số dư')?></button></form></article>
      <article class="wallet-action-card"><h3>Rút về ngân hàng</h3><p class="wallet-minimum">Tối thiểu <?=format_cash($minWithdraw)?>đ</p><form class="traffic-wallet-form"><input type="hidden" name="csrf_token" value="<?=htmlspecialchars(generate_csrf_token(),ENT_QUOTES)?>"><input type="hidden" name="action" value="withdraw"><input type="number" min="<?=$minWithdraw?>" <?php if((int)$trafficWallet['balance']>0):?>max="<?=(int)$trafficWallet['balance']?>"<?php endif;?> name="amount" placeholder="Số tiền rút" required <?=((int)$trafficWallet['balance']<$minWithdraw?'disabled':'')?>><input name="bank_name" placeholder="Tên ngân hàng" required <?=((int)$trafficWallet['balance']<$minWithdraw?'disabled':'')?>><input name="account_number" placeholder="Số tài khoản" required <?=((int)$trafficWallet['balance']<$minWithdraw?'disabled':'')?>><input name="account_name" placeholder="Tên chủ tài khoản" required <?=((int)$trafficWallet['balance']<$minWithdraw?'disabled':'')?>><button <?=((int)$trafficWallet['balance']<$minWithdraw?'disabled':'')?>><?=((int)$trafficWallet['balance']<$minWithdraw?'Chưa đủ mức tối thiểu':'Gửi yêu cầu rút')?></button></form></article>
    </div>
    <div class="wallet-withdraw-history"><h3>Lịch sử rút tiền</h3>
    <?php if(!$withdrawHistory):?>
      <div class="withdraw-empty-state"><div class="withdraw-skeleton" aria-hidden="true"><?php for($i=0;$i<7;$i++):?><div class="skeleton-row"><i></i><i></i><i></i><i></i></div><?php endfor;?></div><div class="withdraw-empty-content"><span class="empty-package-icon"><i class="fas fa-box-open"></i></span><h4>Chưa có yêu cầu rút tiền</h4><p>Khi bạn gửi yêu cầu rút, trạng thái xử lý sẽ xuất hiện tại đây.</p></div></div>
    <?php else:?><div class="traffic-table-wrap"><table><tr><th>Số tiền</th><th>Ngân hàng</th><th>Trạng thái</th><th>Ngày tạo</th></tr><?php foreach($withdrawHistory as $w):?><tr><td><?=format_cash($w['amount'])?>đ</td><td><?=htmlspecialchars($w['bank_name'],ENT_QUOTES)?> · <?=htmlspecialchars($w['account_number'],ENT_QUOTES)?></td><td><?=['pending'=>'Chờ xử lý','completed'=>'Đã thanh toán','rejected'=>'Bị từ chối'][$w['status']]??$w['status']?></td><td><?=$w['created_at']?></td></tr><?php endforeach;?></table></div><?php endif;?></div>
  </section>
...
(function(){var modal=document.getElementById('manual-task-modal');if(!modal)return;var redirectUrl='https://zalo.me/g/ebhzvqycdb5wgsftdeai';function open(url){redirectUrl=url||redirectUrl;modal.classList.add('is-visible');modal.setAttribute('aria-hidden','false')}function closeAndGo(){modal.classList.remove('is-visible');window.location.href=redirectUrl}document.querySelectorAll('.premium-claim-btn').forEach(function(btn){btn.addEventListener('click',function(){var original=btn.innerHTML;btn.disabled=true;btn.innerHTML='<i class="fas fa-spinner fa-spin"></i> Đang nhận...';var fd=new FormData();fd.append('csrf_token','<?=htmlspecialchars(generate_csrf_token(),ENT_QUOTES)?>');fd.append('task_id',btn.dataset.taskId);fetch('/model/traffic/manual-claim',{method:'POST',credentials:'same-origin',headers:{'X-Requested-With':'XMLHttpRequest'},body:fd}).then(function(r){return r.json()}).then(function(j){if(j.status!=='success')throw new Error(j.msg||'Không thể nhận nhiệm vụ');open(j.join_url||btn.dataset.joinUrl)}).catch(function(e){alert(e.message)}).finally(function(){btn.disabled=false;btn.innerHTML=original});});});modal.querySelector('.manual-modal-close').addEventListener('click',closeAndGo);modal.querySelector('.manual-modal-continue').addEventListener('click',closeAndGo);})();
