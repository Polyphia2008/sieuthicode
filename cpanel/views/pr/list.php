...
$listDatatable = $db->get_list(' SELECT * FROM `advertisement` WHERE ' . $where . ' ORDER BY `id` DESC LIMIT ' . $from . ',' . $limit . ' ');
$totalDatatable = $db->num_rows(' SELECT * FROM `advertisement` WHERE ' . $where . ' ORDER BY id DESC ');
$urlDatatable = pagination('?limit=' . $limit . '&shortByDate=' . $shortByDate . '&', $from, $totalDatatable, $limit);
echo '<main id="main-container">
    <div class="content">
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h4 class="page-title fw-semibold fs-18 mb-0"><i class="fa-solid fa-users"></i> Quảng cáo</h4>
        </div>
        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                ...
                <div class="d-flex">
                    <a href="/cpanel/pr/add" class="btn btn-sm btn-primary btn-wave waves-light waves-effect waves-light"><i class="fa fa-plus"></i> Thêm mới</a>
                </div>
            </div>
...
foreach ($listDatatable as $row) {
    ...
    echo '" class="btn btn-sm btn-light" data-bs-toggle="tooltip" title="Xem"> <i class="fa fa-eye"></i> </a>
                                        <a type="button" href="/cpanel/pr/list/update/';
    echo $row['id'];
