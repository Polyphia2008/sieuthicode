function HSDemo() {
	...
	if (headerMode === "single") {
		...
		fisrtElement.innerHTML = '<div class="navbar-dark w-100"> <div class="container">' +
			fisrtElement.firstElementChild.innerHTML +
			"</div> </div>";
		_header2.innerHTML = fisrtElement.innerHTML +
			' <div class="container">' +
...
function getRequestFunctionality(getUrlPrefix, id, getElementType) {
	...
	$("#sub-sub-category-select").empty().append(`<option value="null" selected disabled>---` +
		message +
		`---</option>`);
	$.get({
		url: getUrlPrefix,
...
$(".product-gallery-filter").on("change", function() {
	...
	let categoryId = name == "category_id" ? $(this).val() : getData.data("category-id");
	let vendorId = name == "vendor_id" ? $(this).val() : getData.data("vendor-id");
	let urlParams = [];
