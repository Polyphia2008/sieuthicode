<?php
// statically decompiled from transaction-withdraw.php  [structured; all 1 record(s) structured]
...
if (!$user) {
    ...
    $title = 'Lịch Sử Rút Vật Phẩm- ' . $db->site('title');
    ...
    $listOrder = $db->get_list('SELECT * FROM `withdraw_logs` WHERE ' . $where . ' ORDER BY `id` DESC LIMIT ' . $from . ',' . $sotin1trang . ' ');
    echo '<div class="breadCrumbs">
    <div class="screen">
        <div class="center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a class="text-decoration-none" href="/"><span>Trang chủ</span></a></li>
                <li class="breadcrumb-item active"><a class="text-decoration-none" href=""><span>Lịch sử rút vật phẩm</span></a></li>
            </ol>
        </div>
    </div>
</div>
<div class="screen">
            ...
            <div class="container-account" id="container-account">
                <div class="content-account header-mobile-account">
                    <div class="div-header">
                        ...
                        <h2 class="title-account">Lịch sử rút vật phẩm</h2>
                    </div>
                </div>
    ...
    echo $keyword;
    echo '" placeholder="Tìm kiếm mã giao dịch">
                                        <button><custom-seach>
                                            ...
                                            <div class="group-input input-selectize">
                                                <div class="input-element">
                                                    <select name="service" class="select-component" data-max="1" data-search="false"
                                                        multiple="multiple" placeholder="Danh mục vật phẩm">
                                                        ';
    foreach ($db->get_list('SELECT * FROM `units` WHERE `status` = 1 ORDER BY `stt` ASC') as $sub) {
    ...
    echo '                                                    </select>
                                                ...
                                                <label for="select-telco" class="font-weight-bold">Danh mục vật phẩm</label>
                                            </div>
                                            <div class="group-input input-selectize">
                                                <div class="input-element">
    ...
    echo '" id="to_date" name="to_date" class="input-date-singe" data-drops="up" placeholder="Đến ngày">
                                    ...
                                    <div class="footer-popup-confirm">
                                        <a class="cancel-confirm-nguyennhieu" href="/customer/history/withdraw">Huỷ bỏ</a>
    ...
    foreach ($listOrder as $info) {
        ...
        echo '                            <a class="row-gd" href="/customer/history/withdraw/detail/';
        echo $info['trans_id'];
        echo '">
                                <div>
                                    <p class="ten_row">';
    ...
    echo '                    </div>
                    <div class="paging-account mt-3">
                        <div class="default-paginate">
                            ';
    $total = $db->num_rows('SELECT * FROM `withdraw_logs` WHERE ' . $where . ' ORDER BY `id` DESC ');
    if ($sotin1trang < $total) {
        echo pagination_client('/customer/history/withdraw?keyword=' . $keyword . '&service=' . $service . '&status=' . $status . '&from_date=' . $from_date . '&to_date=' . $to_date . '&', $from, $total, $sotin1trang);
    }
    echo '                        </div>
                    </div>
                </div>
