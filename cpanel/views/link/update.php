...
if (isset($_GET['id']) && is_admin_account($data_user)) {
    $id = Anti_xss($_GET['id']);
    $row = $db->get_row('SELECT * FROM `links` WHERE `id` = \'' . $id . '\' ');
    if (!$row) {
        new Redirect('/cpanel/link/list');
    }
} else {
    new Redirect('/cpanel/link/list');
}
if (isset($_POST['UpdateLink']) && is_admin_account($data_user)) {
    if ($db->site('status_demo') != 0) {
        ...
        if (check_img('image')) {
            ...
            if ($addlogo) {
                $db->update('links', ['image' => $uploads_dir], ' `id` = \'' . $row['id'] . '\' ');
            }
        }
        $isInsert = $db->update('links', ['title' => Anti_xss($_POST['title']), 'link' => Anti_xss($_POST['link']), 'slug' => create_slug(Anti_xss($_POST['title'])), 'status' => Anti_xss($_POST['status'])], ' `id` = \'' . $row['id'] . '\' ');
        if ($isInsert) {
            insetLog($data_user['id'], 'Cập nhật liên kết ' . Anti_xss($_POST['title']) . '');
    ...
    <div class="content">
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">CHỈNH SỬA LIÊN KẾT</h3>
            </div>
            <div class="block-content block-content-full overflow-x-auto">
                <form action="" method="POST" enctype="multipart/form-data">
                    <div class="row mb-4">
                        <div class="row mb-4">
                            <label class="col-sm-4 col-form-label" for="example-hf-email">Tiêu đề:
    ...
    echo ' value="0">OFF</option>
                    ...
                    <a type="button" class="btn btn-danger" href="/cpanel/link/list"><i class="fa fa-fw fa-undo me-1"></i> Quay lại</a>
