<?php
// statically decompiled from footer.php  [structured; all 1 record(s) structured]

echo '<footer id="page-footer" class="bg-body-light">
    <div class="content py-0">
        <div class="row fs-sm">
            <div class="col-sm-6 order-sm-2 mb-1 mb-sm-0 text-center text-sm-end">
                Copyright © 2024 <i class="fa fa-heart text-danger"></i> by <a class="fw-semibold" href="https://sieuthicode.vn" target="_blank">SIEUTHICODE.VN</a>
            </div>
            <div class="col-sm-6 order-sm-1 text-center text-sm-start">
                Phiên bản <a class="fw-semibold text-danger">';
echo $system['version'];
echo '</a>
            </div>
        </div>
    </div>
</footer>
</div>
<script src="/assets/back-end/js/dashmix.app.min-5.9.js"></script>
<script src="/assets/back-end/js/plugins/bootstrap-datepicker/js/bootstrap-datepicker.min.js"></script>
<script src="/assets/back-end/js/plugins/select2/js/select2.full.min.js"></script>
<script src="/assets/back-end/js/plugins/flatpickr/flatpickr.min.js"></script>
<script src="/assets/back-end/js/simple-notify.min.js"></script>
<script src="/assets/back-end/js/chart.js/chart.min.js"></script>
<script>Dashmix.helpersOnLoad([\'js-flatpickr\', \'jq-datepicker\', \'jq-maxlength\', \'jq-select2\', \'jq-rangeslider\', \'jq-pw-strength\']);</script>
</body>

</html>';
