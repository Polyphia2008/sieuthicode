...
if (!empty($_GET['service'])) {
    ...
    $where .= ' AND `name_package` LIKE "%' . $service . '%" ';
}
...
$listLogs = $db->get_list('SELECT * FROM `order_items` WHERE ' . $where . ' ' . $order_by . ' LIMIT ' . $from . ',' . $sotin1trang . ' ');
echo '<main id="main-container">
    <div class="content">
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h4 class="page-title fw-semibold fs-18 mb-0">Thống kê mua vật phẩm</h4>
            <div class="ms-md-1 ms-0">
                <nav>
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item"><a href="#">Mua vật phẩm</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Thống kê mua vật phẩm</li>
                    </ol>
                </nav>
            </div>
        </div>
                    ...
                    <div class="block-content block-content-full d-flex align-items-center justify-content-between">
                        <div class="me-3">
                            <p class="fs-3 fw-medium mb-0">
                                ';
echo format_cash($db->get_row('SELECT COUNT(id) as total FROM order_items WHERE `status` = \'canceled\'')['total'] ?? 0);
echo '                            </p>
                ...
                <a class="block block-rounded block-link-pop" href="javascript:void(0)">
                    <div class="block-content block-content-full d-flex align-items-center justify-content-between">
                        <div class="me-3">
                            <p class="fs-3 fw-medium mb-0">
                                ';
echo format_cash($db->get_row('SELECT COUNT(id) as total FROM order_items WHERE `status` = \'completed\'')['total'] ?? 0);
echo '                            </p>
                            <p class="text-muted mb-0">
                                Đơn hoàn thành
                            </p>
                ...
                <a class="block block-rounded block-link-pop" href="javascript:void(0)">
                    <div class="block-content block-content-full d-flex align-items-center justify-content-between">
                        <div class="me-3">
                            <p class="fs-3 fw-medium mb-0">
                                ';
echo format_cash($db->get_row('SELECT SUM(payment) AS total_amount FROM order_items WHERE `status` = \'completed\'')['total_amount'] ?? 0);
echo 'đ
                            </p>
                            <p class="text-muted mb-0">
                                Đã thanh toán
...
echo '" placeholder="Chọn thời gian" data-mode="range">
                        ...
                        <div class="col-12">
                            <button class="btn btn-hero btn-sm btn-primary"><i class="fa fa-search"></i>
                                Lọc </button>
                            <a class="btn btn-hero btn-sm btn-danger" href="/cpanel/items/orders"><i class="fa fa-trash"></i>
                                Bỏ lọc </a>
                        </div>
                    </div>
                    ...
                    <div class="col-sm-12 col-md-12 mb-3">
                        <div class="pagination-style-1">
                            <div class="d-flex justify-content-center">
                                ';
$total = $db->num_rows('SELECT * FROM `order_items` WHERE ' . $where);
if ($sotin1trang < $total) {
    echo '<center>' . pagination('/cpanel/items/orders?user_id=' . $userid . '&service=' . $service . '&status=' . $status . '&createdate=' . $createdate . '&limit=' . $limit . '&shortByDate=&', $from, $total, $sotin1trang) . '</center>';
}
echo '                            </div>
                        </div>
                    </div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title" id="staticBackdropLabel2">ĐƠN HÀNG MUA VẬT PHẨM [<b id="idOrder">0</b>]
                </h6>
...
<script>
    function modalViewOrder(id) {
        $.ajax({
            url: "/model/admin/modal/order/items",
            method: "POST",
            data: {
