...
$raw=(string)($_GET['token']??'');if(!preg_match('/^[a-f0-9]{64}$/',$raw))traffic_verify_redirect('Mã xác thực nhiệm vụ không hợp lệ');
$hash=hash('sha256',$raw);$db->query('START TRANSACTION');$attempt=$db->get_row("SELECT * FROM `traffic_attempts` WHERE `token_hash`='".$hash."' FOR UPDATE");
...
$updated=$db->update('traffic_attempts',['status'=>'completed','completed_at'=>date('Y-m-d H:i:s')],"`id`='".(int)$attempt['id']."' AND `status`='issued'");
if(!$updated||$db->affected_rows()!==1){$db->query('ROLLBACK');traffic_verify_redirect('Nhiệm vụ đã được xử lý');}
$credited=PlusCredits((int)$attempt['user_id'],(int)$attempt['reward'],'Thưởng hoàn thành nhiệm vụ Traffic #'.$attempt['task_id'],'TRAFFIC_AUTO_'.$attempt['id']);
if(!$credited){$db->query('ROLLBACK');traffic_verify_redirect('Không thể cộng tiền thưởng');}
$db->query('COMMIT');traffic_verify_redirect('Hoàn thành nhiệm vụ! Đã cộng '.format_cash($attempt['reward']).'đ vào số dư',true);
