...
if (isset($_POST['AddLink']) && is_admin_account($data_user)) {
    if ($db->site('status_demo') != 0) {
        ...
        $isInsert = $db->insert('links', ['image' => $url_icon, 'title' => Anti_xss($_POST['title']), 'link' => Anti_xss($_POST['link']), 'slug' => create_slug(Anti_xss($_POST['title'])), 'status' => Anti_xss($_POST['status']), 'created_at' => gettime()]);
        if ($isInsert) {
            insetLog($data_user['id'], 'Thêm liên kết ' . Anti_xss($_POST['title']) . ' vào hệ thống.');
    ...
    <div class="content">
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">THÊM LIÊN KẾT MỚI</h3>
            </div>
