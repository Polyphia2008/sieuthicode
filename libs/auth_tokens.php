 ...
 *  - Token ROTATION mỗi lần restore: sinh raw token mới (random_bytes(32)), UPDATE
 *    hash của CHÍNH row token trong DB, giữ nguyên expires_at tuyệt đối; cookie mới chỉ
 *    được gửi KHI UPDATE thành công — token cũ ngay lập tức không dùng lại được.
 *  - auth_token_issue() kiểm tra kết quả INSERT: thất bại -> không set cookie, trả 0.
...
if (!function_exists('auth_token_issue')) {
    function auth_token_issue($db, $userId, $remember = false, $ip = '')
    {
        ...
        $isInsert = $db->insert('auth_tokens', [
            'user_id' => $userId,
        ...
        if (!$isInsert) {
            // INSERT thất bại -> KHÔNG set cookie, KHÔNG báo thành công.
            return 0;
        }
...
if (!function_exists('auth_token_restore')) {
    function auth_token_restore($db, Session $session)
    {
        ...
        // --- Token rotation: ATOMIC compare-and-swap, fail-closed ---
        // Hai request đồng thời dùng cùng một old token có thể cùng vượt qua
        // SELECT ở trên. Nếu UPDATE chỉ theo `id`, cả hai sẽ ghi đè token của
        // nhau và đều "thành công" -> một phiên bị đánh cắp lặng lẽ. Vì vậy
        // phải hoán đổi nguyên tử với điều kiện `AND token = <old hash>` và
        // chỉ chấp nhận khi affected_rows === 1 (đúng một dòng được cập nhật).
        //
        // Nếu affected_rows !== 1 (0 = token cũ đã bị request khác rotate
        // trước; >1 = bất thường): FAIL CLOSED — không tạo session, không đặt
        // auth_expires_at, không gửi cookie mới, xoá remember_me cookie và
        // trả về guest. expires_at tuyệt đối giữ nguyên, không gia hạn.
        $newRaw = bin2hex(random_bytes(32));
        $newHash = auth_token_hash($newRaw);
        $db->query(
            "UPDATE `auth_tokens` SET `token` = '" . $db->escape($newHash) . "'"
            . " WHERE `id` = '" . (int) $row['id'] . "'"
            . " AND `token` = '" . $db->escape($hash) . "'"
        );
        $swapped = ($db->affected_rows() === 1);
        if (!$swapped) {
            // Race thua hoặc token đã đổi: fail closed.
            auth_token_clear_cookie();
            return '';
        }
        // Chỉ sau khi rotation thành công mới cấp phiên + cookie mới.
        auth_token_send_cookie($newRaw, $expiresAt);
