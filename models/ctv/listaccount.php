...
if (!$user) {
    ...
    if ($data_user['ctv'] != 1) {
        ...
        if (!$query) {
            ...
            $implode = implode('', $sql);
            foreach ($db->get_list('SELECT * FROM `accounts` WHERE `id` != \'0\' ' . $sql_user . ' ' . $sql_type . ' ' . $implode . ' ' . $orderby . ' LIMIT ' . $start . ', ' . $length) as $info) {
                ...
                $json = [];
                $json[] = '<input type="checkbox" data-id="' . $info['id'] . '" name="checkbox"
                ...
                $json[] = '' . $info['sale'] . '%';
                $json[] = $status;
                $json[] = '<a class="btn btn-info btn-sm" href="/ctv/account/edit/' . $info['id'] . '" target="_blank"><i class="fa fa-pen"></i></a>';
            ...
            $output = ['draw' => $_POST['draw'], 'recordsTotal' => $db->num_rows('SELECT * FROM `accounts` WHERE `username_post` = \'' . $data_user['username'] . '\' AND `type_category` = \'' . $type . '\''), 'recordsFiltered' => $db->num_rows('SELECT * FROM `accounts` WHERE `id` != \'0\' ' . $sql_user . ' ' . $sql_type . '  ' . $implode), 'data' => $data];
