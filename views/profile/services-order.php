<?php
// statically decompiled from services-order.php  [structured; all 1 record(s) structured]
...
if (!$user) {
    ...
    if (isset($_GET['code'])) {
        ...
        $row = $db->get_row(' SELECT * FROM `orders` WHERE `code` = \'' . $code . ('\' AND `user_id` = \'' . $data_user['id'] . '\' '));
        if (!$row) {
            new Redirect('/customer/services');
        }
    } else {
        new Redirect('/customer/services');
    }
