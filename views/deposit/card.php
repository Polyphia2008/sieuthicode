...
if (!$user) {
    ...
    $title = 'Lịch Sử Nạp Thẻ Tự Động- ' . $db->site('title');
    ...
    if (!empty($_GET['keyword'])) {
        ...
        $where .= ' AND (`trans_id` = \'' . $keyword . '\' OR `pin` = \'' . $keyword . '\' OR `serial` = \'' . $keyword . '\')';
    }
    $telco = '';
    if (!empty($_GET['telco'])) {
    ...
    if (!empty($_GET['from_date']) && !empty($_GET['to_date'])) {
        ...
        $where .= ' AND `create_date` >= \'' . $start_timestamp . '\' AND `create_date` <= \'' . $end_timestamp . '\' ';
    }
    ...
    <div class="screen">
        <div class="center">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"><a class="text-decoration-none" href=""><span>Lịch sử nạp thẻ tự động</span></a></li>
            </ol>
        </div>
    </div>
</div>
<div class="screen">
            ...
            <div class="container-account" id="container-account">
                <div class="content-account header-mobile-account">
                    <div class="div-header">
                        ...
                        <h2 class="title-account">Lịch sử nạp thẻ</h2>
                    </div>
                </div>
    ...
    echo $keyword;
    echo '" placeholder="Tìm kiếm mã giao dịch, code, serial ...">
                                        <button><custom-seach>
                                        ...
                                        <div class="filter-form">
                                            <div class="group-input input-selectize">
                                                <div class="input-element">
                                                    <select id="select-telco" name="telco" class="select-component" data-max="1" data-search="false"
                                                        multiple="multiple" placeholder="Chọn loại giao dịch">
                                                        <option value="VIETTEL" ';
    echo $telco == 'VIETTEL' ? 'selected' : '';
    echo '>VIETTEL</option>
                                                        <option value="VINAPHONE" ';
    echo $telco == 'VINAPHONE' ? 'selected' : '';
    echo '>VINAPHONE</option>
                                                        <option value="MOBIFONE" ';
    echo $telco == 'MOBIFONE' ? 'selected' : '';
    echo '>MOBIFONE</option>
                                                        <option value="ZING" ';
    echo $telco == 'ZING' ? 'selected' : '';
    echo '>ZING</option>
                                                        <option value="GARENA" ';
    echo $telco == 'GARENA' ? 'selected' : '';
    echo '>GARENA</option>
                                                        <option value="VCOIN" ';
    echo $telco == 'VCOIN' ? 'selected' : '';
    echo '>VCOIN</option>
                                                        <option value="GATE" ';
    echo $telco == 'GATE' ? 'selected' : '';
    echo '>GATE</option>
                                                    </select>
                                                </div>
                                                <label for="select-telco" class="font-weight-bold">Loại giao dịch</label>
                                            </div>
                                            <div class="group-input input-selectize">
                                                <div class="input-element">
    ...
    echo '" id="to_date" name="to_date" class="input-date-singe" data-drops="up" placeholder="Đến ngày">
                                    ...
                                    <div class="footer-popup-confirm">
                                        <a class="cancel-confirm-nguyennhieu" href="/customer/history/card">Huỷ bỏ</a>
    ...
    foreach ($listOrder as $info) {
        echo '                            <a class="row-gd" href="/customer/history/card/detail/';
        echo $info['trans_id'];
        echo '">
                                <div>
                                    <p class="ten_row">';
    ...
    if ($sotin1trang < $total) {
        echo pagination_client('/customer/history/card?keyword=' . $keyword . '&telco=' . $telco . '&status=' . $status . '&from_date=' . $from_date . '&to_date=' . $to_date . '&', $from, $total, $sotin1trang);
    }
    echo '                        </div>
                    </div>
                </div>
